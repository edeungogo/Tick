...
def init_korean_font():
    """
    서버 환경에 한글 폰트가 없어도 구글 나눔고딕 웹폰트를 
    실시간으로 로드하여 matplotlib 지도와 스케일바의 한글 깨짐을 완벽히 해결합니다.
    """
    try:
        ...
        with open("NanumGothic-Bold.ttf", "wb") as f:
            f.write(res.content)
        font_prop = fm.FontProperties(fname="NanumGothic-Bold.ttf")
...
# 막대 형태의 7단계 위험도 스케일 바와 현재 위치를 화살표로 그리는 시각화 함수
def draw_risk_scale_bar(score, current_label):
    ...
    arrow_x = float(score)
    ax.text(arrow_x, 0.4, "🔺\n현재 위험도", ha='center', va='bottom', fontsize=11, color='black', fontweight='bold', fontproperties=font_prop)
